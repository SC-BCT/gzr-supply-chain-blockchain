...
async function initializePage() {
    ...
    try {
        ...
        // 从data.json中提取projectData（它可能是一个字符串）
        const projectDataString = jsonData.projectData;
...
function renderProjectIntro(projectData) {
    const data = projectData || getCurrentData();
    document.getElementById('projectIntro').textContent = data.projectIntro;
}
            ...
            <div class="flex justify-between items-start mb-3">
                <div class="flex-1">
                    ...
                    <div class="flex flex-wrap gap-4 text-sm text-blue-100 mb-2">
                        <span><strong>发表时间：</strong>${paper.time}</span>
                        <span><strong>发表期刊：</strong>${paper.journal}</span>
                        <span><strong>论文作者：</strong>${paper.authors}</span>
                    </div>
                </div>
        ...
        <div class="award-item bg-white bg-opacity-10 rounded-lg p-4 border border-white border-opacity-20" data-id="${award.id}" draggable="true">
            <div class="flex justify-between items-center">
                <div class="flex-1">
                    ...
                    <p class="text-sm text-blue-100">获奖年份：${award.year}</p>
                </div>
            ...
            <div class="flex justify-between items-start">
                <div class="grid grid-cols-3 gap-4 flex-1 items-start">
                    ...
                    <div class="col-span-2 flex flex-col items-start">
                        ...
                        <div class="text-blue-100 whitespace-pre-wrap align-top">${formatMultilineText(student.achievements)}</div>
                    </div>
                </div>
            ...
            <div class="flex justify-between items-center">
                <div class="flex-1">
                    ...
                    <div class="flex gap-4 text-sm text-blue-100">
                        <span><strong>举办地点：</strong>${conference.location}</span>
