...
async function loadPaperData() {
    try {
        ...
        if (response.ok) {
            ...
            // 如果data.json不存在，显示错误信息
            console.warn('data.json文件不存在或无法访问');
        }
    } catch (error) {
...
// 处理GitHub Pages兼容性
function setupGitHubCompatibility() {
    // 检查是否在GitHub Pages环境中
    const isGitHubPages = window.location.hostname.includes('github.io');
    
    if (isGitHubPages) {
        // 在GitHub Pages上，需要确保所有路径都是相对的
        console.log('GitHub Pages环境检测到，启用兼容模式');
        
        // 如果sessionStorage中没有登录状态，检查localStorage
