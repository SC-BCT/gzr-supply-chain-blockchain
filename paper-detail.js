...
function getBasePath() {
    // 如果是GitHub Pages，路径可能是 /username.github.io/repo-name/
    // 或者是根路径 /username.github.io/
    ...
    if (path.includes('.github.io')) {
        ...
        // 移除页面文件名
        if (parts.length > 1 && parts[parts.length - 1].includes('.html')) {
            parts.pop();
        }
        // 如果有仓库名，返回仓库名作为基础路径
...
function showLoadingStates() {
    document.getElementById('loadingPaperInfo').style.display = 'block';
    document.getElementById('loadingPaperDetails').style.display = 'block';
    document.getElementById('loadingHomepageImages').style.display = 'block';
    document.getElementById('loadingKeyImages').style.display = 'block';
}

// 隐藏加载状态，显示内容
function hideLoadingStates() {
    document.getElementById('loadingPaperInfo').style.display = 'none';
    document.getElementById('paperInfoContent').style.display = 'block';
    
    document.getElementById('loadingPaperDetails').style.display = 'none';
    document.getElementById('paperDetailsContent').style.display = 'block';
    
    document.getElementById('loadingHomepageImages').style.display = 'none';
    document.getElementById('homepageContent').style.display = 'block';
    
    document.getElementById('loadingKeyImages').style.display = 'none';
...
function setDefaultContent() {
    ...
    const defaultText = '<p class="text-para">暂无内容，请检查数据文件</p>';
...
async function loadPaperData() {
    ...
    try {
        ...
        if (projectData && projectData.papers) {
            ...
            if (paper) {
                console.log('从localStorage加载论文数据:', paper);
                // 渲染论文基本信息
                document.getElementById('paperTitle').textContent = paper.title || '论文标题';
                document.getElementById('paperJournal').textContent = paper.journal || '未知期刊';
                document.getElementById('paperTime').textContent = paper.time || '未知时间';
                document.getElementById('paperAuthors').textContent = paper.authors || '未知作者';
                return;
            }
        }
        
        // 如果localStorage没有，尝试从data.json加载
        const dataUrl = buildUrl('data.json');
        console.log('尝试加载data.json:', dataUrl);
        
        const response = await fetch(dataUrl);
        if (response.ok) {
            const jsonData = await response.json();
            console.log('data.json加载成功:', jsonData);
            ...
            if (paper) {
                ...
                // 保存到localStorage
                DataManager.save('projectData', jsonData);
            } else {
                console.warn('未找到ID为', currentPaperId, '的论文');
            }
        } else {
            console.warn('data.json文件不存在或无法访问:', response.status);
            // 设置默认值
            document.getElementById('paperTitle').textContent = '论文标题';
            document.getElementById('paperJournal').textContent = '未知期刊';
            document.getElementById('paperTime').textContent = '未知时间';
...
async function loadPaperDetails() {
    console.log('开始加载论文详情数据...');
    try {
        // 先尝试从localStorage加载
        paperDetailsData = DataManager.load('paperDetails', {});
        console.log('从localStorage加载的paperDetails:', paperDetailsData);
        
        // 如果localStorage没有数据，尝试从paperDetails.json加载
        if (!paperDetailsData || Object.keys(paperDetailsData).length === 0) {
            try {
                const paperDetailsUrl = buildUrl('paperDetails.json');
                console.log('尝试加载paperDetails.json:', paperDetailsUrl);
                
                const response = await fetch(paperDetailsUrl);
                if (response.ok) {
                    paperDetailsData = await response.json();
                    console.log('paperDetails.json加载成功:', paperDetailsData);
                } else {
                    console.warn('paperDetails.json文件不存在或无法访问:', response.status);
                    // 创建默认结构
                    paperDetailsData = {};
                }
            } catch (error) {
                console.log('加载paperDetails.json失败，将创建默认结构:', error);
                paperDetailsData = {};
            }
        }
        ...
        console.log('当前论文详情数据:', currentPaperDetails);
...
async function loadImages() {
    ...
    try {
        ...
        console.log('从paperDetails加载的图片 - homepage:', homepageImages);
        console.log('从paperDetails加载的图片 - key:', keyImages);
        
        // 如果paperDetails.json中没有图片数据，尝试从images文件夹加载
        if (homepageImages.length === 0) {
            console.log('尝试从文件夹加载homepage图片...');
            const homepageImgs = await loadImagesFromFolder(`images/papers/${currentPaperId}/homepage/`);
            console.log('从文件夹加载的homepage图片:', homepageImgs);
            
            if (homepageImgs.length > 0) {
                // 保存到paperDetailsData中
                ...
                paperDetailsData[currentPaperId].homepageImages = homepageImgs;
                DataManager.save('paperDetails', paperDetailsData);
            }
        }
        
        if (keyImages.length === 0) {
            console.log('尝试从文件夹加载key图片...');
            const keyImgs = await loadImagesFromFolder(`images/papers/${currentPaperId}/key/`);
            console.log('从文件夹加载的key图片:', keyImgs);
            ...
            if (keyImgs.length > 0) {
                ...
                paperDetailsData[currentPaperId].keyImages = keyImgs;
                DataManager.save('paperDetails', paperDetailsData);
            }
        }
        
        // 重新从paperDetailsData获取图片（可能已被更新）
        const updatedPaperDetails = paperDetailsData[currentPaperId] || {
            homepageImages: [],
            keyImages: []
        };
        
        console.log('最终使用的图片数据:', updatedPaperDetails);
        
        // 渲染图片
        renderImages('homepageImages', updatedPaperDetails.homepageImages || []);
        renderImages('keyImages', updatedPaperDetails.keyImages || []);
...
async function loadImagesFromFolder(folderPath) {
    console.log('从文件夹加载图片:', folderPath);
    const images = [];
    ...
    try {
        ...
        if (images.length === 0) {
            const otherNames = folderPath.includes('homepage') 
                ? ['homepage', 'cover', 'front', 'page1', 'main']
                : ['key', 'figure', 'chart', 'diagram', 'result'];
        ...
        for (let i = 1; i <= 20; i++) {
            ...
            if (await checkImageExists(imagePath) && !images.includes(imagePath)) {
                images.push(imagePath);
                console.log('找到图片:', imagePath);
            }
        }
    ...
    console.log('从文件夹加载到的图片总数:', images.length);
    return images;
}
...
function renderImages(containerId, images) {
    ...
    container.innerHTML = validImages.map((image, index) => {
        // 确保图片路径正确
        let imageSrc = image;
        // 如果图片路径不是完整URL，构建完整URL
        if (!imageSrc.startsWith('http') && !imageSrc.startsWith('data:')) {
            imageSrc = buildUrl(imageSrc);
        }
        
        // 为图片路径添加时间戳，避免浏览器缓存问题
        const cacheBuster = `?t=${new Date().getTime()}`;
        
        return `
            <div class="image-container">
                <div class="image-item bg-gray-100 rounded-lg overflow-hidden" data-image="${imageSrc}">
                    <img src="${imageSrc}${cacheBuster}" alt="论文图片" 
                         class="w-full cursor-pointer"
                         onclick="openImageModal('${imageSrc.replace(/'/g, "\\'")}')"
                         onerror="this.onerror=null; this.style.display='none'; this.parentElement.innerHTML='<div class=\\'p-4 text-center text-red-500\\'>图片加载失败</div>'">
                    ${isLoggedIn ? `
...
function openImageModal(imageSrc) {
    if (elements.modalImage) {
        // 重置缩放
        imageScale = 1.0;
        elements.modalImage.style.transform = `scale(${imageScale})`;
        elements.imageModal.classList.remove('zoomed');
        
        // 设置图片源
        elements.modalImage.src = imageSrc + `?t=${new Date().getTime()}`;
...
function zoomIn() {
    if (imageScale < MAX_SCALE) {
        imageScale = Math.min(MAX_SCALE, imageScale + SCALE_STEP);
        elements.modalImage.style.transform = `scale(${imageScale})`;
        
        if (imageScale > 1.0) {
            elements.imageModal.classList.add('zoomed');
        }
    }
}
...
function zoomOut() {
    if (imageScale > MIN_SCALE) {
        imageScale = Math.max(MIN_SCALE, imageScale - SCALE_STEP);
        elements.modalImage.style.transform = `scale(${imageScale})`;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.addEventListener('error', function(e) {
        ...
        showNotification('页面加载出现错误，请刷新重试', 'error');
    });
});